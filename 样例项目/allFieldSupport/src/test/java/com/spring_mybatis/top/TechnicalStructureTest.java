...
@AnalyzeClasses(packagesOf = AllFieldSupportApp.class, importOptions = DoNotIncludeTests.class)
class TechnicalStructureTest {
    ...
    static final ArchRule respectsTechnicalArchitectureLayers = layeredArchitecture()        .layer("Config").definedBy("..config..").layer("Web").definedBy("..web..")        .optionalLayer("Service").definedBy("..service..")        .layer("Security").definedBy("..security..").layer("Persistence").definedBy("..repository..")        .layer("Domain").definedBy("..domain..")
        ...
        .ignoreDependency(belongToAnyOf(AllFieldSupportApp.class), alwaysTrue())        .ignoreDependency(alwaysTrue(), belongToAnyOf(com.spring_mybatis.top.config.Constants.class,com.spring_mybatis.top.config.ApplicationProperties.class        ));
}
