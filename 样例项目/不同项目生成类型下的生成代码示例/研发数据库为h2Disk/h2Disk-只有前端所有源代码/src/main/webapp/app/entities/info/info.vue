<template>
  <div>
    <h2 id="page-heading" data-cy="InfoHeading">
      <span v-text="$t('sampleProjectH2DiskApp.info.home.title')" id="info-heading">Infos</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('sampleProjectH2DiskApp.info.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'InfoCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-info">
            ...
            <span v-text="$t('sampleProjectH2DiskApp.info.home.createLabel')"> Create a new Info </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && infos && infos.length === 0">
      <span v-text="$t('sampleProjectH2DiskApp.info.home.notFound')">No infos found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('comments')">
              <span v-text="$t('sampleProjectH2DiskApp.info.comments')">Comments</span>
            ...
            <th scope="row" v-on:click="changeOrder('image')">
              <span v-text="$t('sampleProjectH2DiskApp.info.image')">Image</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="sampleProjectH2DiskApp.info.delete.question" data-cy="infoDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-info-heading" v-text="$t('sampleProjectH2DiskApp.info.delete.question', { id: removeId })">
