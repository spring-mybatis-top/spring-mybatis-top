<template>
  <div>
    <h2 id="page-heading" data-cy="StudentHeading">
      <span v-text="$t('sampleProjectH2DiskApp.student.home.title')" id="student-heading">Students</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('sampleProjectH2DiskApp.student.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'StudentCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('sampleProjectH2DiskApp.student.home.createLabel')"> Create a new Student </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && students && students.length === 0">
      <span v-text="$t('sampleProjectH2DiskApp.student.home.notFound')">No students found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('stuName')">
              <span v-text="$t('sampleProjectH2DiskApp.student.stuName')">Stu Name</span>
            ...
            <th scope="row" v-on:click="changeOrder('money')">
              <span v-text="$t('sampleProjectH2DiskApp.student.money')">Money</span>
            ...
            <th scope="row" v-on:click="changeOrder('language')">
              <span v-text="$t('sampleProjectH2DiskApp.student.language')">Language</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'language'"></jhi-sort-indicator>
            </th>
            <th scope="row"><span v-text="$t('sampleProjectH2DiskApp.student.mylesn')">Mylesn</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="student in students" :key="student.id" data-cy="entityTable">
            ...
            <td>{{ student.money }}</td>
            <td v-text="$t('sampleProjectH2DiskApp.Language.' + student.language)">{{ student.language }}</td>
            <td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="sampleProjectH2DiskApp.student.delete.question" data-cy="studentDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-student-heading" v-text="$t('sampleProjectH2DiskApp.student.delete.question', { id: removeId })">
