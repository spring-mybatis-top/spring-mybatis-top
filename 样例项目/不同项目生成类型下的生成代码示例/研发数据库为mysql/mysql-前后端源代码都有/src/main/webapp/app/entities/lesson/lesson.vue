<template>
  <div>
    <h2 id="page-heading" data-cy="LessonHeading">
      <span v-text="$t('sampleProjectMysqlApp.lesson.home.title')" id="lesson-heading">Lessons</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('sampleProjectMysqlApp.lesson.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'LessonCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('sampleProjectMysqlApp.lesson.home.createLabel')"> Create a new Lesson </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && lessons && lessons.length === 0">
      <span v-text="$t('sampleProjectMysqlApp.lesson.home.notFound')">No lessons found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('lesnName')">
              <span v-text="$t('sampleProjectMysqlApp.lesson.lesnName')">Lesn Name</span>
            ...
            <th scope="row" v-on:click="changeOrder('teacherName')">
              <span v-text="$t('sampleProjectMysqlApp.lesson.teacherName')">Teacher Name</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="sampleProjectMysqlApp.lesson.delete.question" data-cy="lessonDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-lesson-heading" v-text="$t('sampleProjectMysqlApp.lesson.delete.question', { id: removeId })">
