<template>
  <div>
    <h2 id="page-heading" data-cy="EmployeeHeading">
      <span v-text="$t('sampleProjectAllFildApp.employee.home.title')" id="employee-heading">Employees</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('sampleProjectAllFildApp.employee.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'EmployeeCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('sampleProjectAllFildApp.employee.home.createLabel')"> Create a new Employee </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && employees && employees.length === 0">
      <span v-text="$t('sampleProjectAllFildApp.employee.home.notFound')">No employees found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('empName')">
              <span v-text="$t('sampleProjectAllFildApp.employee.empName')">Emp Name</span>
            ...
            <th scope="row" v-on:click="changeOrder('salary')">
              <span v-text="$t('sampleProjectAllFildApp.employee.salary')">Salary</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'salary'"></jhi-sort-indicator>
            </th>
            <th scope="row"><span v-text="$t('sampleProjectAllFildApp.employee.myDept')">My Dept</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="sampleProjectAllFildApp.employee.delete.question"
      ...
      <div class="modal-body">
        <p id="jhi-delete-employee-heading" v-text="$t('sampleProjectAllFildApp.employee.delete.question', { id: removeId })">
