<template>
  <div>
    <h2 id="page-heading" data-cy="PersonHeading">
      <span v-text="$t('sampleProjectAllFildApp.person.home.title')" id="person-heading">People</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('sampleProjectAllFildApp.person.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'PersonCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('sampleProjectAllFildApp.person.home.createLabel')"> Create a new Person </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && people && people.length === 0">
      <span v-text="$t('sampleProjectAllFildApp.person.home.notFound')">No people found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('personName')">
              <span v-text="$t('sampleProjectAllFildApp.person.personName')">Person Name</span>
            ...
            <th scope="row" v-on:click="changeOrder('age')">
              <span v-text="$t('sampleProjectAllFildApp.person.age')">Age</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'age'"></jhi-sort-indicator>
            </th>
            <th scope="row"><span v-text="$t('sampleProjectAllFildApp.person.myinfo')">Myinfo</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="sampleProjectAllFildApp.person.delete.question" data-cy="personDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-person-heading" v-text="$t('sampleProjectAllFildApp.person.delete.question', { id: removeId })">
